...
MODEL_NAME_TO_EVAL = "checkpoints/best___32454.dat"

NUM_EVAL_EPISODE = 3
NUM_EVAL_STEPS = 4000

def evaluate(port=8080):
    env = SimstarEnv(track=simstar.TrackName.DutchGrandPrix,port=port,
    synronized_mode=False,speed_up=1,hz=10,
    add_agent=False)

    # env = SimstarEnv(track=simstar.TrackName.HungaryGrandPrix,
    #                  port=port, synronized_mode=True, speed_up=6, hz=10,
    #                  lower_speed_limit=5,
    #                  add_agent=False
    #                  )
    #
    ...
    agent = TD3agent(env, hyprm, insize=insize, device=device)
    agent.to(device)
